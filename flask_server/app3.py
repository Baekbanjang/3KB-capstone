...
def get_gesture_set():
    ...
    if os.path.exists('data/gesture/' + gesture_preset + '/gesture_train_' + code[gesture_code] + '.csv') and os.path.getsize('data/gesture/' + gesture_preset + '/gesture_train_' + code[gesture_code] + '.csv') > 0:
        file = np.genfromtxt('data/gesture/' + gesture_preset +
    ...
    while cap.isOpened():
        ...
        if (isStop):
            np.savetxt('data/gesture/' + gesture_preset +
...
def get_pose_set():
    ...
    if os.path.exists('data/pose/'+pose_preset+'/pose_angle_train_' + code[pose_code] + '.csv') and os.path.getsize('data/pose/'+pose_preset+'/pose_angle_train_' + code[pose_code] + '.csv') > 0:
        file = np.genfromtxt('data/pose/'+pose_preset+'/pose_angle_train_' +
    ...
    while cap.isOpened():
        ...
        if result.pose_landmarks is not None:
            ...
            cv2.putText(img, f'Current Pose: {code[pose_code]}', 
                        (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
        if (isStop):
            np.savetxt('data/pose/'+pose_preset+'/pose_angle_train_' +
