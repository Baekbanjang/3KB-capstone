...
def get_pose_set():
    ...
    if os.path.exists('flask_server/data/pose/'+pose_preset+'/pose_angle_train_' + code[pose_code] + '.csv') and os.path.getsize('flask_server/data/pose/'+pose_preset+'/pose_angle_train_' + code[pose_code] + '.csv') > 0:
        file = np.genfromtxt('flask_server/data/pose/'+pose_preset+'/pose_angle_train_' +
                             code[pose_code] + '.csv', delimiter=',')
    else:
        file = np.empty((0, 9))
    ...
    while cap.isOpened():
        ...
        if result.pose_landmarks is not None:
            ...
            pose_code_int = int(pose_code)  # pose_code를 정수로 변환
            data = np.append(angles,  pose_code_int)
            ...
            cv2.putText(img, f'Current Pose: {code[pose_code]}', 
                        (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
        if (isStop):
            np.savetxt('flask_server/data/pose/'+pose_preset+'/pose_angle_train_' +
                       code[pose_code] + '.csv', file, delimiter=',')
        ret, jpeg = cv2.imencode('.jpg', img)
        frame = jpeg.tobytes()
        yield (b'--frame\r\n'
               b'Content-Type: image/jpeg\r\n\r\n' + frame + b'\r\n')
...
def pose_gen():
    ...
    if os.path.exists('flask_server/data/pose/' + pose_preset + '/pose_angle_train.csv') and os.path.getsize('flask_server/data/pose/' + pose_preset + '/pose_angle_train.csv') > 0:
        file = np.genfromtxt('flask_server/data/pose/' +
                             pose_preset + '/pose_angle_train.csv', delimiter=',')
    else:
        file = np.empty((0, 9))
    ...
    while cap.isOpened():
        ...
        if result.pose_landmarks is not None:
            ...
            angle4 = calculateAngle(landmarks[mp_pose.PoseLandmark.RIGHT_ELBOW.value],
                                    landmarks[mp_pose.PoseLandmark.RIGHT_WRIST.value], landmarks[mp_pose.PoseLandmark.RIGHT_PINKY.value])
            angle5 = calculateAngle(landmarks[mp_pose.PoseLandmark.LEFT_HIP.value],
                                    landmarks[mp_pose.PoseLandmark.LEFT_KNEE.value], landmarks[mp_pose.PoseLandmark.LEFT_ANKLE.value])
            angle6 = calculateAngle(landmarks[mp_pose.PoseLandmark.RIGHT_HIP.value],
                                    landmarks[mp_pose.PoseLandmark.RIGHT_KNEE.value], landmarks[mp_pose.PoseLandmark.RIGHT_ANKLE.value])
            angle7 = calculateAngle(landmarks[mp_pose.PoseLandmark.LEFT_SHOULDER.value],
                                    landmarks[mp_pose.PoseLandmark.LEFT_HIP.value], landmarks[mp_pose.PoseLandmark.LEFT_KNEE.value])
            angle8 = calculateAngle(landmarks[mp_pose.PoseLandmark.RIGHT_SHOULDER.value],
                                    landmarks[mp_pose.PoseLandmark.RIGHT_HIP.value], landmarks[mp_pose.PoseLandmark.RIGHT_KNEE.value])

            # 각도 데이터를 배열에 저장
            # 각도 계산 결과를 배열에 추가. 필요한 각도 수에 맞게 조정하세요.
            pose_array = np.array(
...
def process_body_data():
    ...
    if request.method == 'POST':
        ...
        if 'delete_button_value' in request.form:
            ...
            if os.path.exists(file_path):
                os.remove(file_path)
            return render_template('GetMovementDataSet.html', message="파일을 삭제합니다.")
